...
proof -
  ...
  proof -
    fix q
    assume q_in: "q \<in> A"
    show "ring_prime\<^bsub>P\<^esub> (const_poly q)"
      using hprime[OF q_in] unfolding P_eq .
  qed
  have S_closure_eq: "S = ring_mult_submonoid_closure P (const_poly ` A)"
  proof -
    have "S = ring_mult_submonoid_closure (K[X]) (const_poly ` A)"
      by (rule S_eq)
    also have "\<dots> = ring_mult_submonoid_closure P (const_poly ` A)"
      using P_eq by simp
    finally show ?thesis .
  qed
  ...
  proof -
    ...
    then obtain q where q_in: "q \<in> A" and p_def: "p = const_poly q"
      by blast
    show "ring_prime\<^bsub>P\<^esub> p"
      unfolding p_def by (rule hprimeP[OF q_in])
  qed
